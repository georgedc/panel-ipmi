...
function _doCsrfInsert (name, token)
{
    // for ajax
    ...
    XMLHttpRequest.prototype.onsend = function (data) {
        this.setRequestHeader (name, token);
    };

    // for form
    var domForms = document.forms;
    for (var i = 0; i < domForms.length; i++) {
        var csrfField = document.createElement ('input');
        csrfField.setAttribute ('type' , 'hidden');
        csrfField.setAttribute ('name' , name);
        csrfField.setAttribute ('value', token);
        domForms[i].appendChild (csrfField);
    }
}
