        ...
        <section class="section-card h-100">
            ...
            <div class="section-body">
                <form method="post" action="<?php echo routeUrl('/servers/create'); ?>">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4"><label class="form-label"><?php echo htmlspecialchars(__('servers.ipmi_port')); ?></label><input class="form-control" type="number" name="ipmi_port" value="623" style="-moz-appearance:textfield;appearance:textfield;"></div>
    ...
    <?php endif; ?>
        <section class="section-card">
            ...
            <div class="px-3 py-3 border-bottom d-flex flex-wrap gap-2 align-items-center">
                <input id="filter-search" type="text" class="form-control" style="max-width:260px;" placeholder="<?php echo htmlspecialchars(__('servers.filter_search')); ?>">
                ...
                <span id="filter-count" class="ms-auto" style="font-size:.82rem;color:var(--panel-muted);"></span>
            </div>
            <?php if (!empty($isAdmin)) : ?>
                ...
                <table class="table soft-table align-middle" id="servers-table">
                    <thead>
                        <tr>
                            ...
                            <th><?php echo htmlspecialchars(__('server.last_check')); ?></th>
                        ...
                        <?php else : ?>
                            <?php foreach ($servers as $server) : ?>
                                ...
                                <tr class="server-row"
                                    ...
                                    <td><span class="font-monospace" style="font-size:.88rem;color:var(--panel-text);"><?php echo htmlspecialchars((string) ($server['ip_address'] ?? '')); ?></span></td>
                                    <td><span id="server-status-<?php echo (int) ($server['id'] ?? 0); ?>" class="status-pill <?php echo htmlspecialchars($status); ?>"><span class="status-dot"></span><span class="status-pill-label"><?php echo htmlspecialchars(strtoupper($status)); ?></span></span></td>
                                    ...
                                    <td><?php
                                        $loc = (string) ($server['location'] ?? '');
                                        echo $loc !== '' ? htmlspecialchars(mb_convert_case($loc, MB_CASE_TITLE, 'UTF-8')) : htmlspecialchars(__('servers.na'));
                                    ?></td>
                                    <td class="text-end">
                                        <div class="panel-actions">
                                            <a class="panel-btn" href="<?php echo routeUrl('/server', ['id' => (int) ($server['id'] ?? 0)]); ?>" title="<?php echo htmlspecialchars(__('mvc.servers_open_detail')); ?>">
                                                <i class="material-icons-round">visibility</i>
                                            </a>
                                            <?php if (!empty($isAdmin)) : ?>
                                                <a class="panel-btn" href="<?php echo routeUrl('/servers/edit', ['id' => (int) ($server['id'] ?? 0)]); ?>" title="<?php echo htmlspecialchars(__('mvc.servers_edit')); ?>">
                                                    <i class="material-icons-round">edit</i>
                                                </a>
                                                <form method="post" action="<?php echo routeUrl('/servers/delete'); ?>" style="display:inline;" onsubmit="return confirm('<?php echo htmlspecialchars(__('mvc.servers_delete_confirm')); ?>');">
                                                    <?php echo \App\Http\Csrf::getHiddenField(); ?>
                                                    <input type="hidden" name="server_id" value="<?php echo (int) ($server['id'] ?? 0); ?>">
                                                    <button class="panel-btn panel-btn-danger" type="submit" title="<?php echo htmlspecialchars(__('mvc.servers_delete')); ?>">
